...
async fn main() -> Result<()> {
    ...
    println!("   ✓ Profile picture uploaded (ID: {})\n", profile_pic_upload.file_id);
    ...
    println!("   ✓ Track: {} - {}", spotify_track.track_name, spotify_track.artist_name);
    println!("   ✓ Album: {}", spotify_track.album_name.as_ref().unwrap());
    println!("   ✓ Provider: {:?}", spotify_track.provider);
    println!("   ✓ Background image: {} ({}x{})", bg_image_upload.file_id, 1080, 1920);
    ...
    println!("   ✓ Found {} profiles matching 'music'", search_results.len());
    ...
    println!("   ✓ Retrieved background image: {} bytes\n", retrieved_bg.len());
